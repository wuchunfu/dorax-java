...
public class ExcelExportHelper {
    ...
    private final Logger logger = LoggerFactory.getLogger(ExcelExportHelper.class);
     ...
     * header、excelList规则如下：
     * header、excelList中的Bean必须对应（javaBean的属性顺序）：如下
     * header：姓名、年龄、性别、班级
     * Bean：name、age、sex、class
     *
     ...
     * 如有图片请转换为byte[]
     * header、excelList规则如下：
     * header、excelList中的Bean必须一一对应(javaBean的属性顺序)：如下
     * header：姓名、年龄、性别、班级
     * Bean：name、age、sex、class
     *
     * @param header     表格属性列名数组
    ...
    public void exportExcelForBigDataAndSave(String[] header, List<Object> excelList, String sheetTitle, String flag, String filePath, String fileName) {
        ...
        String fileNameAddSuffix;
        for (int i = 0; i < books.size(); i++) {
            HSSFWorkbook book = books.get(i);
            fileNameAddSuffix = getFileName(fileName) + "_0" + i;
            // 保存Excel文件
            saveExcel(book, filePath, fileNameAddSuffix);
        }
    }
        ...
        while (it.hasNext()) {
            ...
            for (short i = 0; i < fields.length; i++) {
                ...
                try {
                    ...
                    e.printStackTrace();
                    logger.info("<<< Error creating Excel data list. >>>");
                }
            }
        }
        logger.info("<<< Excel data filled successfully! >>>");
    }
    ...
    private String getFileName(String fileName) {
        if (fileName == null || "".equals(fileName)) {
            ...
            fileName = sdf.format(date) + Math.abs(random.nextInt() * 1000000);
        }
        return fileName;
    }
    ...
    private void saveExcel(HSSFWorkbook book, String filePath, String fileName) {
        ...
        try {
            ...
            book.write(out);
            logger.info("<<< Excel file saved successfully, save path: " + "\\" + fileName + ".xls >>>");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void zipExcelAndSave(List<HSSFWorkbook> books, String filePath, String zipName, String excelName) {
        ...
        try {
            ...
            logger.info("<<< Excel file saved successfully, save path: " + filePath + "\\" + zipName + ".xls >>>");
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("<<< Failed to save excel file. >>>");
        } finally {
